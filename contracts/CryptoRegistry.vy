# @version 0.3.0
"""
@title Kagla CryptoSwap Registry
...
def get_D(_pool: address) -> uint256:
    return KaglaPool(_pool).D()


@view
@external
def get_gamma(_pool: address) -> uint256:
    return KaglaPool(_pool).gamma()
...
def get_fees(_pool: address) -> uint256[4]:
    ...
    @return Pool fee as uint256 with 1e10 precision
            ...
            Out fee
    """
    return [KaglaPool(_pool).fee(), KaglaPool(_pool).admin_fee(), KaglaPool(_pool).mid_fee(), KaglaPool(_pool).out_fee()]
