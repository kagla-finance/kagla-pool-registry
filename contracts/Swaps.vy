...
def _get_exchange_amount(
    ...
    if is_underlying and (_registry == self.registry or Registry(_registry).is_meta(_pool)):
        return KaglaPool(_pool).get_dy_underlying(i, j, _amount)

    return KaglaPool(_pool).get_dy(i, j, _amount)
...
def _exchange(
    ...
    if is_underlying and _registry == self.factory_registry and Registry(_registry).is_meta(_pool):
        ...
        if base_coins == empty(address[2]):
            base_coins = [KaglaPool(_pool).coins(0), KaglaPool(_pool).coins(1)]
    ...
    if is_underlying:
        KaglaPool(_pool).exchange_underlying(i, j, _amount, _expected, value=eth_amount)
    else:
        KaglaPool(_pool).exchange(i, j, _amount, _expected, value=eth_amount)
...
def exchange_multiple(
    ...
    for i in range(1,5):
        ...
        if params[2] == 1:
            KaglaPool(swap).exchange(convert(params[0], int128), convert(params[1], int128), amount, 0, value=eth_amount)
        elif params[2] == 2:
            KaglaPool(swap).exchange_underlying(convert(params[0], int128), convert(params[1], int128), amount, 0, value=eth_amount)
        elif params[2] == 3:
...
def set_calculator(_pool: address, _calculator: address) -> bool:
    ...
    @dev Used to calculate `get_dy` for a pool
    @param _pool Pool address
    @param _calculator `KaglaCalc` address
    @return bool success
    """
